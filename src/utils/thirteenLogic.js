...
export const validateArrangement = (rows) => {
    ...
    const details = {
        front: frontHand.type,
        middle: middleHand.type,
        back: backHand.type,
    };

    if (front.length !== 3 || middle.length !== 5 || back.length !== 5) {
        return { isValid: false, message: '牌墩数量错误！请确保牌墩数量为 头道:3, 中道:5, 后道:5。', details: {} };
    }
    
    if (compareHands(frontHand, middleHand) > 0) {
        return { isValid: false, message: '头道牌型大于中道，不符合规则！', details };
    }
    if (compareHands(middleHand, backHand) > 0) {
        return { isValid: false, message: '中道牌型大于后道，不符合规则！', details };
    }

    return { isValid: true, message: '牌型合法', details };
};
